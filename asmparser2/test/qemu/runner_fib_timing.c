...
// fib() -- naive recursive fib(40) makes 331,160,281 calls, far too many
// to actually simulate under QEMU's software emulation in a reasonable
// time. Instead this measures real Xtensa cycles (via CCOUNT, the same
// register the sc_examples corpus's own millis()/elapseMillis() __ASM__
// functions use) for two much smaller, tractable depths, checks the
// resulting cycles-per-call figures agree (proving it's a real, linear,
// per-call cost and not a fluke), and projects fib(40)'s real-hardware
...
int main()
{
    ...
    /* The two cycles-per-call figures should agree closely (same code,
       ...
       esp32s3 QEMU machines) mostly landed within 3-6% but occasionally
       spiked to ~11% (likely periodic timer-interrupt jitter in the
       runtime, not the fib() codegen itself) -- 15% comfortably covers
       that observed noise range while still catching a genuinely broken
       (non-linear) measurement, e.g. one off by 2x. */
    double ratio = perCall1 > perCall2 ? perCall1 / perCall2 : perCall2 / perCall1;
    printf("cycles/call agreement: %.2fx (want <= 1.15x)\n", ratio);
    if (ratio > 1.15)
    {
    ...
    double projectedCycles = avgPerCall * (double)FIB_N3_CALLS;
    double projectedSeconds = projectedCycles / (double)ESP32_CLOCK_HZ;
    printf("projected fib(%d): %ld calls * %.3f cycles/call = %.0f cycles -> %.2f s at %.0f MHz\n",
           FIB_N3, FIB_N3_CALLS, avgPerCall, projectedCycles, projectedSeconds,
