...
int main()
{
    ...
    for (int i = 0; i < NUM_EXT_SLOTS; i++)
    {
        ...
        *(uint32_t *)(script_code + ext_slot_offset[i]) = content;
        printf("patched ext '%s' slot @%u -> %p\n", ext_name[i], ext_slot_offset[i], (void *)(uintptr_t)content);
    }
    fflush(stdout);

#if HAS_FOOTER
    printf("calling footer() -- constructs Balls[20], each via rand()x5 + float division x5...\n");
    fflush(stdout);
    callVoid(script_code + FOOTER_ENTRY_OFFSET);
    printf("footer() returned OK\n");
    fflush(stdout);
#endif

    printf("calling diag() -- clamps num, then printfln(\"numberof balls:%%d\",num)...\n");
    fflush(stdout);
    callVoid(script_code + DIAG_ENTRY_OFFSET);
